...
INSERT INTO `detil_nota` VALUES ('NOA00001','O001',10,30000,'3x1'),('NOA00001','O002',5,25000,'1x1'),('NOA00002','O001',10,30000,'2x1'),('NOA00002','O004',6,1000,'3x3');
...
INSERT INTO `detil_resep` VALUES ('RSP00001','O001',10,'3x1'),('RSP00001','O002',5,'1x1'),('RSP00002','O001',1,'1x1'),('RSP00003','O001',5,'2x1'),('RSP00003','O004',5,'3x3');
...
INSERT INTO `detil_tindakan` VALUES ('DAF00001','TD01',2,35000),('DAF00002','TD01',1,35000),('DAF00002','TD02',1,120000),('DAF00007','TD03',1,200000);
...
INSERT INTO `kwitansi` VALUES ('KWT00001','2016-06-10','DAF00004'),('KWT00002','2016-06-10','DAF00006'),('KWT00003','2016-06-10','DAF00003'),('KWT00004','2016-06-13','DAF00005'),('KWT00005','2016-06-13','DAF00007'),('KWT00006','2016-06-13','DAF00002');
...
INSERT INTO `nota` VALUES ('NOA00001','2016-06-10','RSP00001'),('NOA00002','2016-06-13','RSP00003');
...
INSERT INTO `pasien` VALUES ('P00001','Hendri Pratama','Jakarta','1994-02-02','Pria',22,'Jl.Raya Parung-Ciputat, Bojongsari, Depok','085695152330','Islam','A','Mahasiswa','Hen'),('P00002','asd','asd','2016-05-09','Wanita',2,'asd','21','Protestan','B','asd','we'),('P00003','Fandi','Cipulir','1994-10-21','Pria',22,'Jl.Cipulir','02351561','Islam','B','Mahasiswa','munarif'),('P00004','Husni','Bintaro','1994-03-22','Pria',22,'TMII','008','Islam','A','Mahasiswa','Giri'),('P00005','humisar','jakarta','1980-06-09','Pria',36,'jalan ciledug','087484848343','Islam','A','dosen','giri');
...
INSERT INTO `petugas` VALUES ('PTG001','Wawan','1965-05-17','Pria','Jl.Bojongsari','085214597845','22222'),('PTG002','hendri','2010-02-02','Pria','sawangan','087747548574','22222'),('PTG003','Husni Outlaw','1994-03-22','Pria','Taman Mangu Indah ','085771446292','220394'),('PTG004','Outlaw','1994-03-22','Pria','TMI','085771446292','1122334455');
...
INSERT INTO `resep` VALUES ('RSP00001','2016-06-10','DAF00004'),('RSP00002','2016-06-10','DAF00006'),('RSP00003','2016-06-13','DAF00002');
...
CREATE VIEW `laporanpendatan` AS 
  select `x`.`tgl_kwitansi` AS `TanggalKwt`,`e`.`nm_petugas` AS `petugas`,`d`.`nm_pasien` AS `NamaPasien`,(((`a`.`biaya_daftar` + `a`.`biaya_dokter`) + (select ifnull(sum((`detil_tindakan`.`banyaknya_tindakan` * `detil_tindakan`.`biaya_tindakan`)),0) from `detil_tindakan` where (`detil_tindakan`.`no_daftar` = `a`.`no_daftar`))) + (select ifnull(sum((`detil_nota`.`jml_obat_nota` * `detil_nota`.`harga`)),0) from (`nota` join `detil_nota`) where ((`nota`.`no_resep` = `z`.`no_resep`) and (`nota`.`no_nota` = `detil_nota`.`no_nota`)))) AS `Biaya_Keseluruhan` from (((((((`resep` `z` left join `pendaftaran` `a` on((`z`.`no_daftar` = `a`.`no_daftar`))) left join `nota` `b` on((`b`.`no_resep` = `z`.`no_resep`))) left join `dokter` `f` on((`a`.`kd_dokter` = `f`.`kd_dokter`))) left join `pasien` `d` on((`a`.`no_pasien` = `d`.`no_pasien`))) left join `petugas` `e` on((`a`.`kd_petugas` = `e`.`kd_petugas`))) left join `surat_sehat` `k` on((`k`.`no_daftar` = `a`.`no_daftar`))) left join `kwitansi` `x` on((`x`.`no_daftar` = `a`.`no_daftar`))) group by `a`.`no_daftar`;
...
#
# Trigger "PenguranganStock"
#

DROP TRIGGER IF EXISTS `PenguranganStock`;
CREATE DEFINER='root'@'localhost' TRIGGER `hens`.`PenguranganStock` AFTER INSERT ON `hens`.`detil_nota`
  FOR EACH ROW update obat set stock = stock - new.jml_obat_nota
